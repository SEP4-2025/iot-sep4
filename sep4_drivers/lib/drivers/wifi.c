...
WIFI_ERROR_MESSAGE_t wifi_command_TCP_receive(uint8_t *data, uint16_t maxLength, uint16_t *receivedLength)
{
    char expectedPrefix[] = "+IPD,";
    uint8_t buffer[10];
    uint16_t len = 0;
    int i = 0;

    // Wait for "+IPD," prefix
    while (i < strlen(expectedPrefix))
    {
        uart_receive_array_blocking(USART_WIFI, buffer, 1);
        if (buffer[0] == expectedPrefix[i])
            i++;
        else
            i = 0; // Restart matching if mismatch
    }

    // Now read the length part after "+IPD,"
    len = 0;
    while (1)
    {
        uart_receive_array_blocking(USART_WIFI, buffer, 1);
        if (buffer[0] == ':')
            break;
        len = len * 10 + (buffer[0] - '0');
    }

    if (len > maxLength)
        return WIFI_FAIL; // Too much data for buffer

    // Read the actual data
    uart_receive_array_blocking(USART_WIFI, data, len);
    *receivedLength = len;
